...
from xysz.main_biget import cof_main, data_delet_middle, data_delet_slow, fetch_redis_data
...
def FB_strategy(self):
    ...
    try:
        time.sleep(10)
        all_middle_data, all_slow_data, ws_data = fetch_redis_data()
        now = datetime.now(pytz.timezone('Asia/Shanghai'))

        for symbol, price in ws_data.items():
            cache_key = f"kline_{symbol}_{now}"
            ...
            # 获取当前时间（东八区）
            current_timestamp = f"{now.strftime('%Y-%m-%d %H:%M:%S')}+08:00"
            close1 = float(price)
            ...
            five_key = f"FB{symbol}_{five_now}"
...
def KC_strategy(self):
    ...
    try:
        ...
        for symbol, price in ws_data.items():
            # cache_key = f"kline_{symbol}_{ws_data['timestamp'].replace(' ', '_').replace(':', '-')}"
            # if cache.get(cache_key):
            #     # print(f"数据已处理过: {cache_key}")
            #     return
