...
def calculate_trading_params(fast_his_data, symbol, plan, exchange, mode, period=None, atr_period=None):
    global account
    # 计算斐波那契和ATR水平
...
def define_KC_strategy(exchange, symbol, time, fast_his_data, grid_value):
    ...
    try:
        ...
        if grid_value == 3:
            ...
            # print(fast_atr)
            log_message = f"KC_{exchange}_{symbol}_{time},中轨2:{Medium_2},中轨1:{Medium_track},close3:{close3},close2:{close2},close1:{close1},adx:{adx_last},atr:{fast_atr},量比:{volume_ratio},成交量{volume}"
            print(log_message)
