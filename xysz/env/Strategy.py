...
FRside = {}
MTside = {}
kcsymbol_was_in_zone = {}
smartsymbol_was_in_zone = {}
# kcsarmtsymbol_was_in_zone = {}
Ferrysymbol_was_in_zone = {}
MTsymbol_was_in_zone = {}
Martinside = {}
Martinsymbol_was_in_zone = {}
KCsmartside = {}
KCsamtrside = {}
signalswitch = {}
...
def calculate_bollinger_bands_talib(
    close_prices: Union[list, np.ndarray],
    period: int = 20,
    nbdevup: float = 2.0,
    nbdevdn: float = 2.0,
    matype: int = 0
    ...
    """  
    示例:
        upper, middle, lower = calculate_bollinger_bands_talib(close_prices, period=20, nbdevup=2)
    """
    # 确保输入为numpy数组
    ...
    if len(close_array) < period:
        raise ValueError(f"数据长度({len(close_array)})必须大于周期({period})")
...
# 止损1：马丁参考价（近30根波动超过阈值时返回当前价）
def df_Martin_stop_price(exchange, plan, symbol, time, mode, fast_data):
    try:
        ...
        close30 = fast_data.iloc[-30]['close']

        if mode == 1:
            diff = abs(close1 - close30)
            percentage = (diff / min(close1, close30)) * 100
            if percentage > 0.7:
                return close1
        return None
    except Exception as e:
        print(f"MartinStop_{symbol}_{time}报错: {e}")
        return None


def calculate_atr_levels(ohlc_data: pd.DataFrame, 
                                  period: int = None, 
                                  atr_period: int = None) -> float:
    """计算period周期内的ATR均值"""
    # 确保数据足够计算
    if len(ohlc_data) < atr_period:
        raise ValueError(f"数据不足计算ATR，需要至少 {atr_period} 根K线，当前只有 {len(ohlc_data)} 根")
    
    # 计算ATR
    atr_values = talib.ATR(ohlc_data['high'], ohlc_data['low'], ohlc_data['close'], 
                          timeperiod=atr_period)
    
    # 计算最近period个ATR值的均值
    recent_atr_values = atr_values.tail(period)
    atr_mean = recent_atr_values.mean()
    lastatr = atr_values.iloc[-1]
    
    return atr_mean,lastatr
...
def calculate_ma(df, period=None):

    # 确保价格数据是数值类型
    prices = pd.to_numeric(df['close'], errors='coerce')
    
    # 计算MA（简单移动平均）
    ma_values = talib.MA(prices, timeperiod=period, matype=0)
...
def calculate_trading_params(fast_his_data, symbol, plan, exchange, time, mode, period=None, atr_period=None):
    ...
    # 计算斐波那契和ATR水平
    fib_0, fib_1, fib_2, fib_3, fib_4, fib_5, fib_6, atr_mean, fast_his_data = calculate_atr_fibonacci_levels(
        fast_his_data, period=period, atr_period=atr_period
    )
    fast_atr = fast_his_data['atr'].iloc[-1]
    close_price = fast_his_data['close'].iloc[-1]
    close_price2 = fast_his_data['close'].iloc[-2]
    ...
    multiple = 1
    # 根据ATR值确定交易参数
    if symbol in DPO_VALUES_1:
        if mode == 2:
            dpo_values = DPO_VALUES_1[symbol]
        else:
            dpo_values = DPO_VALUES_2
        DPO_VALUES_1 = DPO_VALUES_1[symbol]
    ...
    # print(fast_atr,atr_mean)
    if fast_atr < atr_mean * 0.5 or fast_atr > atr_mean * 1.5:
        dpo = 10
        tp = 0.5
        dpo2 = 10
        
    if fib_3 < fast_atr < fib_4:
        multiple = 3
    if fib_4 < fast_atr < fib_5:
        multiple = 2
    tp = fast_atr * 2

    martinprice = df_Martin_stop_price(exchange, plan, symbol, time, mode, fast_his_data)
    if martinprice is not None:
        stoploss1 = abs(martinprice - close_price)
    # 布林带函数需要收盘价序列，不能传整张 DataFrame（含 timestamp 字符串会报 float 转换错误）
    bbprice = None
    upper, middle, lower = calculate_bollinger_bands_talib(
        fast_his_data['close'].values, period=20, nbdevup=2
    )
    upper_last, lower_last = upper[-1], lower[-1]
    if close_price2 > upper_last and close_price < upper_last:
        bbprice = close_price
    elif close_price2 < lower_last and close_price > lower_last:
        bbprice = close_price
    if bbprice is not None:
        stoploss2 = abs(close_price - bbprice)
    
    stoploss3 = calculate_avg_price_range(fast_his_data, period=10)
...
def calculate_atr_fibonacci_levels(ohlc_data: pd.DataFrame, 
    ...
    return fib_0, fib_1, fib_2, fib_3, fib_4, fib_5, fib_6, atr_mean, recent_data
...
def define_grid_strategy(exchange, symbol, fast_his_data, middle_his_data, grid_value, grid_pct=1.3):
    global FBside,middle_current_time
    try:
        ...
        if grid_value == 3:
            ...
            if last_params is None:
                # 首次发送信号，并存储参数组合
                # 存储新记录
                cache.set(base_key, current_params, timeout=120)
...
def define_KC_strategy(exchange, symbol, time, fast_his_data, grid_value):
    global KCside,kcsymbol_was_in_zone
    try:
        ...
        if grid_value == 3:
            plan = 'KC'
            kc_upper,kc_lower,Medium_track,_ = calculate_kc_channel(fast_his_data, ema_period=100, atr_period=14)
            _, _ ,vi_ratios = calculate_vi_manual(fast_his_data,period=6)
            # print(vi_ratios)
            vi_ratio1 = vi_ratios[-1]
            # print(vi_ratio1)
            adx_value = calculate_adx(fast_his_data, period=10)
            rsi_values = calculate_rsi_with_talib(fast_his_data, period=10)
            # vi_ratio1 = vi_ratios[-1]
            adx_last = float(adx_value.iloc[-1])
            rsi_value = rsi_values[-1]
            # 获取最近3个值
            adx_last_3 = adx_value.iloc[-3:].values
            rsi_last_3 = rsi_values[-3:]

            # 判断是否连续递增
            adx_increasing = adx_last_3[0] < adx_last_3[1] < adx_last_3[2]
            rsi_increasing = rsi_last_3[0] < rsi_last_3[1] < rsi_last_3[2]

            # 判断是否连续递减
            adx_decreasing = adx_last_3[0] > adx_last_3[1] > adx_last_3[2]
            rsi_decreasing = rsi_last_3[0] > rsi_last_3[1] > rsi_last_3[2]

            # 初始化当前symbol的状态（如果不存在）
            if symbol not in kcsymbol_was_in_zone:
                kcsymbol_was_in_zone[symbol] = False

            # 判断是否在区间内
            was_in_zone = kcsymbol_was_in_zone[symbol]
            is_in_zone = 0.85 <= vi_ratio1 <= 1.15 

            # 状态变化判断
            if is_in_zone != was_in_zone:
                if is_in_zone: 
                    if adx_increasing and rsi_increasing:
                        mode = 2 
                    else:
                        return exchange, adx_last
                else: 
                    if adx_decreasing and rsi_decreasing:
                        mode = 1 
                    else:
                        return exchange, adx_last

                # 更新状态
                kcsymbol_was_in_zone[symbol] = is_in_zone

            else: 
                if 0.85 <= vi_ratio1 <= 1.15:
                    mode = 1 
                else:
                    mode = 2
        ...
        return exchange, adx_last if 'adx_last' in locals() else 1



"""KC_smart设置信号判断"""
def define_KC_smartstrategy(exchange, symbol, time, fast_his_data, grid_value):
    global KCsmartside,smartsymbol_was_in_zone
    try:
        ...
        if grid_value == 4:
            plan = 'kc_smart'
            atr_period = 10
            kc_upper,kc_lower,Medium_track,_ = calculate_kc_channel(fast_his_data, ema_period=100, atr_period=14, multiplier=1)
            ...
            # 初始化当前symbol的状态（如果不存在）
            if symbol not in smartsymbol_was_in_zone:
                smartsymbol_was_in_zone[symbol] = False

            # 判断是否在区间内
            was_in_zone = smartsymbol_was_in_zone[symbol]
            is_in_zone = 0.85 <= vi_ratio1 <= 1.15 
            ...
            if is_in_zone != was_in_zone:
                ...
                # 更新状态
                smartsymbol_was_in_zone[symbol] = is_in_zone
...
def df_ferry_strategy(exchange, plan, symbol, time, fast_data, Ferryv, period):
    global FRside,Ferrysymbol_was_in_zone
    try:
        # print(exchange, symbol, len(fast_data), grid_value)
        ...
        adx_last = 1
        mode = None
        dpo, dpo2, tp, sl = None, None, None, None
        multiple = 1
        multiple2 = 1
        side = None
        FRside[symbol] = None
        ...
        four_row = fast_data.iloc[-4]
        close2 = prev_row['close']
        close3 = thiry_row['close']
        close4 = four_row['close']

        # print(f"{plan}时间戳打印:{exchange}, {symbol}, {time},{signal_time}")
        fast_data = calculate_volume_ratio(fast_data, period=480)
        volume_ratio = fast_data['volume_ratio'].iloc[-1]
        volume = fast_data['volume'].iloc[-1]
        fib_0, fib_1, fib_2, fib_3, fib_4, fib_5, fib_6, atr_mean, fast_data = calculate_atr_fibonacci_levels(
        fast_data, period=480, atr_period=14
            )
        fast_atr = fast_data['atr'].iloc[-1]

        ma5_values = calculate_ma(fast_data,period=period)
        ...
        ema5_value1 = ma5_values.iloc[-1]
        ema5_value2 = ma5_values.iloc[-2]
        ...
        open2 = last2_row['open']
        low2 = last2_row['low']
        high2 = last2_row['high']
        current_date = pd.Timestamp(buy_date)
        dt = datetime.strptime(buy_date, "%Y-%m-%d %H:%M:%S")
        formatted_time = dt.strftime("%H:%M")
        # print(ema5_value1,ema5_value2,close1,close2)
        adx_value = calculate_adx(fast_data, period=10)
        rsi_values = calculate_rsi_with_talib(fast_data, period=10)
        # vi_ratio1 = vi_ratios[-1]
        adx_last = float(adx_value.iloc[-1])
        rsi_value = rsi_values[-1]
        # 获取最近3个值
        ...
        rsi_decreasing = rsi_last_3[0] > rsi_last_3[1] > rsi_last_3[2]
        # 初始化当前symbol的状态（如果不存在）
        if symbol not in Ferrysymbol_was_in_zone:
            Ferrysymbol_was_in_zone[symbol] = False
        # 获取当前symbol的状态
        was_in_zone = Ferrysymbol_was_in_zone[symbol]
        is_in_zone = 0.85 <= vi_ratio1 <= 1.15
        if is_in_zone != was_in_zone:
            if is_in_zone: 
                if adx_increasing and rsi_increasing:
                ...
                if adx_decreasing and rsi_decreasing:
                    ...
                    return 
            # 更新状态
            Ferrysymbol_was_in_zone[symbol] = is_in_zone
        else: 
            if 0.85 <= vi_ratio1 <= 1.15:
                mode = 1 
            else:
        ...
        if close2 > open2 and close3 < close2 and close2 > ema5_value2:
            FRside[symbol] = 'OPEN_LONG'
        elif open2 > close2 and close3 > close2 and close2 < ema5_value2:
            FRside[symbol] = 'OPEN_SHORT'
        atrvalue = fib_5 > fast_atr > fib_1
        side = FRside.get(symbol, None)
        if side is not None and atrvalue:
            base_key = f"{exchange}_{plan}_{symbol}_{time}_{close1}"
            full_key = f"{base_key}_{plan}_{mode}_{side}_{tp}_{sl}"
            current_params = f"{plan}_{mode}_{side}_{tp}_{sl}"
            # 获取缓存中的历史记录（只存储最新的一条）
            last_params = cache.get(base_key)
            # print(fast_atr)
            log_message = f"FR_{exchange}_{symbol}_{time}_{signal_time},close3:{close3},close2:{close2},close1:{close1},adx:{adx_last},atr:{fast_atr},量比:{volume_ratio},成交量{volume}"
            print(log_message)
            # CSV文件路径
            today = datetime.now().date()
            csv_file = f"./FRsignal{today}_{time}.csv"
            
            if last_params is None:
                # 存储新记录
                cache.set(base_key, current_params, timeout=120)
                # print(f"震荡趋势信号:{full_key}")
                append_to_csv(csv_file, [full_key, signal_time,close4, close3, close2, close1, adx_last,fib_1, fast_atr,fib_5, volume_ratio, volume])
                return 0
            else:
                if last_params != current_params:
                    # print(f"震荡趋势有变化")
            ...
            print(f"{exchange}, {symbol}, {time},{side}, {vi_ratio2},{fib_1},{fast_atr},{fib_5}")
    except Exception as e:
        print(f"FR_{symbol}_{time}报错: {e}")
        return 0
    

def df_MoneyToad_strategy(exchange, plan, symbol, time, fast_data, MT_v):
    global MTside,MTsymbol_was_in_zone
    try:
        # print(exchange, symbol, len(fast_data), MT_v)
        ...
        rsi_decreasing = rsi_last_3[0] > rsi_last_3[1] > rsi_last_3[2]
        # 初始化当前symbol的状态（如果不存在）
        if symbol not in MTsymbol_was_in_zone:
            MTsymbol_was_in_zone[symbol] = False
        # 获取当前symbol的状态
        was_in_zone = MTsymbol_was_in_zone[symbol]
        is_in_zone = 0.85 <= vi_ratio1 <= 1.15
        # 如果是首次进入或首次离开
        if is_in_zone != was_in_zone:
            if is_in_zone:  # 首次进入
                ...
                if adx_decreasing and rsi_decreasing:
                    ...
                    return
            # 更新当前symbol的状态
            MTsymbol_was_in_zone[symbol] = is_in_zone
        else: 
            if 0.85 <= vi_ratio1 <= 1.15:
                mode = 1 
            else:
        ...
        lowprice = min(close2, open2)
        highprice = max(close2, open2)
        if lowprice - low2 > 8 :
            MTside[symbol] = 'OPEN_LONG'
        elif high2 - highprice > 8 :
            MTside[symbol] = 'OPEN_SHORT'
        side = MTside.get(symbol, None)
        if side is not None :
            base_key = f"{exchange}_{plan}_{symbol}_{time}_{close1}"
            full_key = f"{base_key}_{plan}_{mode}_{side}_{tp}_{sl}"
            current_params = f"{plan}_{mode}_{side}_{tp}_{sl}"
            # 获取缓存中的历史记录（只存储最新的一条）
            last_params = cache.get(base_key)
            # log_message = f"MT_{exchange}_{symbol}_{time}_{signal_time},close2:{close2},close1:{close1}"
            # print(log_message)
            # CSV文件路径
            today = datetime.now().date()
            csv_file = f"./MTsignal{today}_{time}.csv"
            
            if last_params is None:
                # 存储新记录
                cache.set(base_key, current_params, timeout=120)
                # print(f"震荡趋势信号:{full_key}")
                append_to_csv(csv_file, [full_key, signal_time, close2, close1])
                return 0
            else:
                if last_params != current_params:
                    # print(f"震荡趋势有变化")
            ...
            print(f"{exchange}, {symbol}, {time},{side}, {vi_ratio2}")
    except Exception as e:
        print(f"MT_{symbol}_{time}报错: {e}")
        return 0


def df_Martin_signal(exchange, plan, symbol, time, mode, fast_data):
    """
    马丁方向信号：
    - mode=1 震荡：近30根涨跌超 0.7% 时逆向开仓
    - mode=2 趋势：近30根涨跌超 0.7% 时顺向开仓
    """
    try:
        if len(fast_data) < 139:
            return None
        ...
        close30 = float(fast_data.iloc[-30]['close'])
        diff = abs(close1 - close30)
        percentage = (diff / min(close1, close30)) * 100
        if percentage <= 0.7:
            return None
        ...
        if mode == 1:
            return 'OPEN_SHORT' if close1 > close30 else 'OPEN_LONG'
        if mode == 2:
            return 'OPEN_LONG' if close1 > close30 else 'OPEN_SHORT'
        return None
    except Exception as e:
        print(f"MartinSignal_{symbol}_{time}报错: {e}")
        return None


def df_Martin_strategy(exchange, plan, symbol, time, fast_data, Martinv):
    """马丁模式主策略：VI 判定震荡/趋势 + 近30根波动触发开仓信号。"""
    global Martinside, Martinsymbol_was_in_zone
    try:
        if len(fast_data) < 139:
            return {
                'is_consolidation': False,
        ...
        if symbol not in Martinsymbol_was_in_zone:
            Martinsymbol_was_in_zone[symbol] = False
        was_in_zone = Martinsymbol_was_in_zone[symbol]
        is_in_zone = 0.85 <= vi_ratio1 <= 1.15
        ...
        if is_in_zone != was_in_zone:
            if is_in_zone:
                ...
                if adx_decreasing and rsi_decreasing:
                    ...
                    return
            Martinsymbol_was_in_zone[symbol] = is_in_zone
        else:
            mode = 1 if is_in_zone else 2

        side = df_Martin_signal(exchange, plan, symbol, time, mode, fast_data)
        if side is not None:
            Martinside[symbol] = side

        side = Martinside.get(symbol, None)
        if side is not None:
            base_key = f"{exchange}_{plan}_{symbol}_{time}_{close1}"
            full_key = f"{base_key}_{plan}_{mode}_{side}_{tp}_{sl}_v{Martinv}"
            current_params = f"{plan}_{mode}_{side}_{tp}_{sl}"
            last_params = cache.get(base_key)
            today = datetime.now().date()
            csv_file = f"./Martinsignal{today}_{time}.csv"
            log_message = (
                f"Martin_{exchange}_{symbol}_{time}_{signal_time},"
                f"close2:{close2},close1:{close1},mode:{mode},side:{side},"
                f"adx:{adx_last},vi:{vi_ratio1}"
            )
            print(log_message)

            if last_params is None:
                cache.set(base_key, current_params, timeout=120)
                append_to_csv(csv_file, [full_key, signal_time, close2, close1, mode, side, adx_last, vi_ratio1])
                return 0
            else:
                if last_params != current_params:
                    cache.set(base_key, current_params, timeout=120)
            ...
            print(f"{exchange}, {symbol}, {time},{side}, mode:{mode}, vi:{vi_ratio2}")
    except Exception as e:
        print(f"Martin_{symbol}_{time}报错: {e}")
        return 0
...
def define_KC_samtrstrategy(exchange, symbol, time, fast_data, grid_value):
    ...
    try:
        ...
        if grid_value == 6:
            ...
            plus_vi, minus_vi,vi_ratios = calculate_vi_manual(fast_data,period=6)
            # vi_ratio1 = vi_ratios[-1]
            minus_vi = minus_vi[-1]
            plus_vi = plus_vi[-1]
            adx_last = float(adx_value.iloc[-1])
            ...
            # 震荡趋势判断
            if adx_last > 25 and plus_vi > minus_vi:
                mode = 2
            elif 25 < adx_last < 30 :
                mode = 1
            else:
