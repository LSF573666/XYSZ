...
def FB_strategy(self, ws_data):
    ...
    try:
        ...
        print(ws_type)
        symbol = ws_type
        slow_keys = list(all_middle_data.keys())
        print(slow_keys)
        ...
        if ws_type in slow_keys:
            ...
            # 确保 middle_his_data 是 DataFrame
            ...
            # print(middle_his_data.tail(2))
            # 确保列名匹配
            # middle_his_data = middle_his_data.copy()  # 假设 middle_his_data 和 middle_his_data 结构类似
            # 根据行数选择要合并的数据

            # 提取交集列
            # middle_cols = kline_data[middle_his_data.columns.intersection(kline_data.columns)]
            slow_cols = kline_data[middle_his_data.columns.intersection(kline_data.columns)]

            # 设置新索引（假设索引是时间戳）
            # middle_cols.index = [middle_his_data.index[-1] + 1]
            slow_cols.index = [middle_his_data.index[-1] + 1]

            # 合并数据
            # middle_his_data = pd.concat([middle_his_data, middle_cols], ignore_index=False)
            middle_his_data = pd.concat([middle_his_data, slow_cols], ignore_index=False)
            # print(middle_his_data.tail(2))
            # print(f"数据合并成功")
            

            # supports, resistances, median_line, (dense_low, dense_high), atr_slope = identify_support_resistance(middle_his_data)
            ...
            strategy_result,tide,mode,atr_slope = define_grid_strategy(symbol, middle_his_data, FB_key)
...
def KC_strategy(self, ws_data):
    ...
    try:
        ...
        ws_key = f"kCline_{ws_type}_{ws_data['timestamp'].replace(' ', '_').replace(':', '-')}"
        if cache.get(ws_key):
            # print(f"数据已处理过: {cache_key}")
            return
